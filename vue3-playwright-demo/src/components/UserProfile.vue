<template>
  <v-container>
    ...
    <!-- 用户信息 -->
    <v-row justify="center" v-else-if="user && !errorMessage">
      <v-col cols="12" md="6">
        <v-card class="pa-4" elevation="3">
          ...
          <v-card-actions>
            <v-btn color="primary" @click="fetchUser"> Reload </v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>

    <!-- 错误提示，确保 Playwright 测试可以找到 -->
    <v-row justify="center" v-if="errorMessage" class="error-container">
      <v-col cols="12" md="6">
...
const fetchUser = async () => {
  ...
  try {
    ...
    console.log("Error Message Set:", errorMessage.value); // ✅ 添加日志
  } finally {
    loading.value = false;
  }
};
